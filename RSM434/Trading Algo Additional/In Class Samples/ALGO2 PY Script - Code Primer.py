...
s.headers.update({'X-API-key': 'W9OPB2TD'}) # assigns the API key "V4PYCGCW" to the header of "s", so every message we send to RIT contains the API key ; you will use the API key for your RIT
...
def main():
    ...
    ticker_list = ['CNR','AC','ALG'] # creates a list of ticker symbols that are traded in the case which will be cycled in the for loop below
