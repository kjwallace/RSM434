...
while tick > 5 and tick < 295 and not shutdown:
        ...
        for i in range(3):
            spreads[i] = get_spread[ticker_list[i]]
        Target_Stock = ticker_list[spreads.index(max(spreads))]
            
        position = get_position()  
        Order_Num = (MAX_LONG_EXPOSURE - position[0])/ORDER_LIMIT
        best_bid, best_ask = get_bid_ask(Target_Stock)
        if max(spreads) > Min_Spread:
            buy_sell(best_ask, best_bid,Target_Stock,Order_Num)
