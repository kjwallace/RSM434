...
s = requests.Session()
s.headers.update({'X-API-key': 'WODPIUKI'}) # Make sure you use YOUR API Key
    ...
    while status == 'ACTIVE':        
        ...
        if gross_position < MAX_EXPOSURE_GROSS and net_position > MAX_SHORT_EXPOSURE_NET and net_position < MAX_LONG_EXPOSURE_NET:
            ...
            if market_prices[0, 0] + market_prices[1, 0] > market_prices[2, 1]: 
                if abs(market_prices[0, 0] + market_prices[1, 0] - market_prices[2, 1]) > MIN_SPREAD:
            ...
            if market_prices[0, 1] + market_prices[1, 1] < market_prices[2, 0]: 
                if abs(market_prices[0, 1] + market_prices[1, 1] - market_prices[2, 1]) > MIN_SPREAD:
